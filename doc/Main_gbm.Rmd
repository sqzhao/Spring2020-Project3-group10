...
Fiducial points are stored in matlab format. In this case, we select half face points as fiducial points, 23 points in total. In this step, we read them and store them in a list.
...
```
In this case, we have 38.2 % accuracy when we use 250 as number of trees in our GBM function. 
...
```
Time for constructing training features= 0.39 s 
Time for constructing testing features= 0.08 s 
Time for training model= 78.33 s 
